...
const EaudouceNourriturePage = () => {
  ...
  const [categoryTitle, setCategoryTitle] = useState<string>("Nourriture pour Poissons d'Eau Douce");
  const [categoryDescription, setCategoryDescription] = useState<string>(
    "Découvrez notre sélection de nourritures adaptées à tous les poissons d'eau douce, pour une alimentation équilibrée et une santé optimale."
  );
  ...
  const handleCategoryChange = (category: Category) => {
    ...
    setSelectedSubCategories([category.slug]);
    setSelectedBrands([]);
    setSelectedBrandIds([]);
    setPriceRange([0, 800]);
    setPriceInput([0, 800]);
  ...
  const handleBrandChange = (brand: Brand) => {
    setSelectedBrands(prev => {
      const newBrands = prev.includes(brand.slug)
  ...
  // Fonction pour gérer le changement de filtre de promotion
  const handlePromoChange = (checked: boolean) => {
    setShowPromosOnly(checked);
    setPromoOnly(checked);
  };

  // Fonction pour gérer le changement de description de produit
  ...
  // Fonction pour gérer le changement de contenu éditable
...
export default EaudouceNourriturePage;
...
function DebugPriceIdPanel({ product }) {
  ...
  const refresh = async () => {
    ...
    const { data } = await supabase
      .from("editable_content")
      .select("content_key, content")
      .like("content_key", `%product_${product.id}%stripe_price_id%`);
    setAllKeys(data || []);
  ...
  const handleCreateBasePriceId = async () => {
    ...
    if (existing) {
      result = await supabase
        .from("editable_content")
        .update({ content: basePriceIdInput.trim() })
        .eq("content_key", key);
    } else {
      result = await supabase
        .from("editable_content")
        .insert({ content_key: key, content: basePriceIdInput.trim() });
    }
    if (result.error) {
      setActionMsg("Erreur lors de la création/mise à jour : " + result.error.message);
    } else {
      setActionMsg("✅ price_id de base enregistré !");
      setBasePriceIdInput("");
      await refresh();
    }
    setLoading(false);
  ...
  const baseKey = allKeys.find(k => k.content_key === `product_${product.id}_stripe_price_id`);
      ...
      {loading ? (
        ...
        <>
          <div className="mb-1 text-xs">
            <b>Clés price_id trouvées dans Supabase :</b>
            <ul className="ml-4 list-disc">
              {allKeys.length === 0 && <li className="text-red-500">Aucune clé trouvée</li>}
              {allKeys.map(k => (
                <li key={k.content_key}>
                  <span className="font-mono bg-gray-200 px-1 py-0.5 rounded">{k.content_key}</span> : <span className="font-mono text-blue-700">{k.content || <span className="text-red-500">(vide)</span>}</span>
                </li>
              ))}
            </ul>
          </div>
          <div className="mb-1 text-xs">
            <b>stripe_price_id sélectionné par le helper :</b>
            <span className={selectedPriceId ? "text-green-700 font-mono ml-2" : "text-red-700 font-mono ml-2"}>
              {selectedPriceId || "❌ Aucun trouvé"}
            </span>
          </div>
          {/* Champ + bouton pour créer le price_id de base si manquant ou vide */}
          {(!baseKey || !baseKey.content) && (
            <div className="mt-2 flex items-center gap-2">
              <input
                type="text"
                placeholder="Coller ici le price_id Stripe (ex: price_1P0...)"
              ...
              {actionMsg && <span className="ml-2 text-xs" style={{ color: actionMsg.startsWith('✅') ? 'green' : 'red' }}>{actionMsg}</span>}
            </div>
          )}
        </>
      )}
